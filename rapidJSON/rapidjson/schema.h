...
public:
    ...
    virtual void TooLong(const Ch* str, uint length, uint expected) = 0;
    virtual void TooShort(const Ch* str, uint length, uint expected) = 0;
    virtual void DoesNotMatch(const Ch* str, uint length) = 0;

    virtual void DisallowedItem(uint index) = 0;
    virtual void TooFewItems(uint actualCount, uint expectedCount) = 0;
    virtual void TooManyItems(uint actualCount, uint expectedCount) = 0;
    virtual void DuplicateItems(uint index1, uint index2) = 0;

    virtual void TooManyProperties(uint actualCount, uint expectedCount) = 0;
    virtual void TooFewProperties(uint actualCount, uint expectedCount) = 0;
    virtual void StartMissingProperties() = 0;
    virtual void AddMissingProperty(const SValue& name) = 0;
    virtual bool EndMissingProperties() = 0;
    virtual void PropertyViolations(ISchemaValidator** subvalidators, uint count) = 0;
    virtual void DisallowedProperty(const Ch* name, uint length) = 0;
    ...
    virtual void NotAllOf(ISchemaValidator** subvalidators, uint count) = 0;
    virtual void NoneOf(ISchemaValidator** subvalidators, uint count) = 0;
    virtual void NotOneOf(ISchemaValidator** subvalidators, uint count) = 0;
    virtual void MultipleOneOf(uint index1, uint index2) = 0;
...
public:
    ...
    bool RawNumber(const Ch* str, uint len, bool) {
        WriteBuffer(kNumberType, str, len * sizeof(Ch));
        return true;
    }

    bool String(const Ch* str, uint len, bool) {
        WriteBuffer(kStringType, str, len * sizeof(Ch));
        return true;
    }

    bool StartObject() { return true; }
    bool Key(const Ch* str, uint len, bool copy) { return String(str, len, copy); }
...
struct SchemaValidationContext {
    ...
    ~SchemaValidationContext() {
        ...
        if (patternPropertiesValidators) {
            for (uint i = 0; i < patternPropertiesValidatorCount; i++) {
    ...
    uint patternPropertiesValidatorCount;
    const SchemaType** patternPropertiesSchemas;
    uint patternPropertiesSchemaCount;
    PatternValidatorType valuePatternValidatorType;
    PatternValidatorType objectPatternValidatorType;
    uint arrayElementIndex;
    bool* propertyExist;
    bool inArray;
...
public:
        ...
        nullable_(false)
    {
        ...
        const ValueType* dependencies = GetMember(value, GetDependenciesString());
        {
            ...
            if (allProperties.Size() > 0) {
                ...
                for (uint i = 0; i < propertyCount_; i++) {
    ...
    ~Schema() {
        ...
        if (patternProperties_) {
            for (uint i = 0; i < patternPropertyCount_; i++)
    ...
    RAPIDJSON_FORCEINLINE bool EndValue(Context& context) const {
        ...
        if (context.patternPropertiesValidatorCount > 0) {
            ...
            uint count = context.patternPropertiesValidatorCount;
        ...
        if (context.validatorCount > 0) {
            if (allOf_.schemas)
                for (uint i = allOf_.begin; i < allOf_.begin + allOf_.count; i++)
            ...
            if (anyOf_.schemas) {
                for (uint i = anyOf_.begin; i < anyOf_.begin + anyOf_.count; i++)
                    if (context.validators[i]->IsValid())
                        goto foundAny;
            ...
            if (oneOf_.schemas) {
                bool oneValid = false;
                uint firstMatch = 0;
                for (uint i = oneOf_.begin; i < oneOf_.begin + oneOf_.count; i++)
                    if (context.validators[i]->IsValid()) {
                        if (oneValid) {
    ...
    bool String(Context& context, const Ch* str, uint length, bool) const {
        ...
        if (minLength_ != 0 || maxLength_ != uint(~0)) {
            uint count;
    ...
    bool StartObject(Context& context) const {
        ...
        if (patternProperties_) { // pre-allocate schema array
            uint count = patternPropertyCount_ + 1; // extra for valuePatternValidatorType
    ...
    bool Key(Context& context, const Ch* str, uint len, bool) const {
    ...
    bool EndObject(Context& context, uint memberCount) const {
        RAPIDJSON_SCHEMA_PRINT(Method, "Schema::EndObject");
        if (hasRequired_) {
            context.error_handler.StartMissingProperties();
            for (uint index = 0; index < propertyCount_; index++)
        ...
        if (hasDependencies_) {
            ...
            for (uint sourceIndex = 0; sourceIndex < propertyCount_; sourceIndex++) {
                const Property& source = properties_[sourceIndex];
                if (context.propertyExist[sourceIndex]) {
                    if (source.dependencies) {
                        context.error_handler.StartMissingDependentProperties();
                        for (uint targetIndex = 0; targetIndex < propertyCount_; targetIndex++)
    ...
    bool EndArray(Context& context, uint elementCount) const {
...
#define RAPIDJSON_STRING_(name, ...) \
    static const ValueType& Get##name##String() {\
        ...
        static const ValueType v(s, static_cast<uint>(sizeof(s) / sizeof(Ch) - 1));\
        return v;\
    }
...
private:
    ...
    struct SchemaArray {
        ...
        uint begin; // begin index of context.validators
        uint count;
    };
    ...
    static void AssignIfExist(uint& out, const ValueType& value, const ValueType& name) {
        if (const ValueType* v = GetMember(value, name))
            if (v->IsUint64() && v->GetUint64() <= uint(~0))
                out = static_cast<uint>(v->GetUint64());
    }
    ...
    static bool IsPatternMatch(const RegexType* pattern, const Ch *str, uint) {
    ...
    void CreateSchemaValidators(Context& context, const SchemaArray& schemas, const bool inheritContinueOnErrors) const {
        for (uint i = 0; i < schemas.count; i++)
            context.validators[schemas.begin + i] = context.factory.CreateSchemaValidator(*schemas.schemas[i], inheritContinueOnErrors);
    }

    // O(n)
    bool FindPropertyIndex(const ValueType& name, uint* outIndex) const {
        uint len = name.GetStringLength();
    ...
    struct Property {
        ...
        uint dependenciesValidatorIndex;
        bool* dependencies;
        bool required;
    };
    ...
    uint validatorCount_;
    uint notValidatorIndex_;

    Property* properties_;
    const SchemaType* additionalPropertiesSchema_;
    PatternProperty* patternProperties_;
    uint patternPropertyCount_;
    uint propertyCount_;
    uint minProperties_;
    uint maxProperties_;
...
struct TokenHelper {
    RAPIDJSON_FORCEINLINE static void AppendIndexToken(Stack& documentStack, uint index) {
        *documentStack.template Push<Ch>() = '/';
        char buffer[21];
        size_t length = static_cast<size_t>((sizeof(uint) == 4 ? u32toa(index, buffer) : u64toa(index, buffer)) - buffer);
...
public:
    ...
    virtual const SchemaDocumentType* GetRemoteDocument(const Ch* uri, uint length) = 0;
...
public:
    ...
    */
    explicit GenericSchemaDocument(const ValueType& document, const Ch* uri = 0, uint uriLength = 0,
        ...
        currentError_()
    {
        ...
        if (pointer.GetTokenCount() == 0) {
            ...
            SchemaErrorValue(kSchemaErrorStartUnknown, PointerType(), sb.GetString(), static_cast<uint>(sb.GetSize() / sizeof(Ch)));
        }
    ...
    void SchemaErrorValue(const SchemaErrorCode code, const PointerType& location, const Ch* value, uint length) {
      currentError_ = GValue(kObjectType);
    ...
    void SchemaErrorPointer(const SchemaErrorCode code, const PointerType& location, const Ch* value, uint length, const PointerType& pointer) {
      currentError_ = GValue(kObjectType);
      currentError_.AddMember(GetValueString(), GValue(value, length, *allocator_).Move(), *allocator_);
      currentError_.AddMember(GetOffsetString(), static_cast<uint>(pointer.GetParseErrorOffset() / sizeof(Ch)), *allocator_);
      AddCurrentError(code, location);
    }
  ...
  private:
    ...
    void AddErrorInstanceLocation(GValue& result, const PointerType& location) {
      ...
      GValue instanceRef(sb.GetString(), static_cast<uint>(sb.GetSize() / sizeof(Ch)), *allocator_);
...
#define RAPIDJSON_STRING_(name, ...) \
    static const StringRefType& Get##name##String() {\
        static const Ch s[] = { __VA_ARGS__, '\0' };\
        static const StringRefType v(s, static_cast<uint>(sizeof(s) / sizeof(Ch) - 1)); \
        return v;\
    }
...
#undef RAPIDJSON_STRING_
    ...
    static OpenApiVersion GetOpenApiVersion(const ValueType& document) {
        ...
        static uint len = internal::StrLen<Ch>(kVersion30String);
    ...
    bool HandleRefSchema(const PointerType& source, const SchemaType** schema, const ValueType& v, const ValueType& document, const UriType& id) {
        ...
        if (itr->value.IsString()) {
            uint len = itr->value.GetStringLength();
            if (len == 0)
                    ...
                    if (len <= 1 || s[1] == '/') {
                        ...
                        if (!relPointer.IsValid())
                            ...
                            if (const ValueType *pv = relPointer.Get(*base)) {
                                ...
                                for (uint i = 0; i < relPointer.GetTokenCount(); i++)
...
public:
    ...
    */
    ...
    void TooLong(const Ch* str, uint length, uint expected) {
        AddNumberError(kValidateErrorMaxLength,
            ValueType(str, length, GetStateAllocator()).Move(), SValue(expected).Move());
    }
    void TooShort(const Ch* str, uint length, uint expected) {
        AddNumberError(kValidateErrorMinLength,
            ValueType(str, length, GetStateAllocator()).Move(), SValue(expected).Move());
    }
    void DoesNotMatch(const Ch* str, uint length) {
        currentError_.SetObject();
        currentError_.AddMember(GetActualString(), ValueType(str, length, GetStateAllocator()).Move(), GetStateAllocator());
        AddCurrentError(kValidateErrorPattern);
    }

    void DisallowedItem(uint index) {
        currentError_.SetObject();
        currentError_.AddMember(GetDisallowedString(), ValueType(index).Move(), GetStateAllocator());
        AddCurrentError(kValidateErrorAdditionalItems, true);
    }
    void TooFewItems(uint actualCount, uint expectedCount) {
        AddNumberError(kValidateErrorMinItems,
            ValueType(actualCount).Move(), SValue(expectedCount).Move());
    }
    void TooManyItems(uint actualCount, uint expectedCount) {
        AddNumberError(kValidateErrorMaxItems,
            ValueType(actualCount).Move(), SValue(expectedCount).Move());
    }
    void DuplicateItems(uint index1, uint index2) {
    ...
    void TooManyProperties(uint actualCount, uint expectedCount) {
        AddNumberError(kValidateErrorMaxProperties,
            ValueType(actualCount).Move(), SValue(expectedCount).Move());
    }
    void TooFewProperties(uint actualCount, uint expectedCount) {
    ...
    void PropertyViolations(ISchemaValidator** subvalidators, uint count) {
        for (uint i = 0; i < count; ++i)
            MergeError(static_cast<GenericSchemaValidator*>(subvalidators[i])->GetError());
    }
    void DisallowedProperty(const Ch* name, uint length) {
        currentError_.SetObject();
    ...
    void NotAllOf(ISchemaValidator** subvalidators, uint count) {
    ...
    void NoneOf(ISchemaValidator** subvalidators, uint count) {
        AddErrorArray(kValidateErrorAnyOf, subvalidators, count);
    }
    void NotOneOf(ISchemaValidator** subvalidators, uint count) {
        AddErrorArray(kValidateErrorOneOf, subvalidators, count);
    }
    void MultipleOneOf(uint index1, uint index2) {
...
#define RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(method, arg2)\
    for (Context* context = schemaStack_.template Bottom<Context>(); context != schemaStack_.template End<Context>(); context++) {\
        ...
        if (context->validators)\
            for (uint i_ = 0; i_ < context->validatorCount; i_++)\
                static_cast<GenericSchemaValidator*>(context->validators[i_])->method arg2;\
        if (context->patternPropertiesValidators)\
            for (uint i_ = 0; i_ < context->patternPropertiesValidatorCount; i_++)\
...
#define RAPIDJSON_SCHEMA_HANDLE_VALUE_(method, arg1, arg2) \
    ...
    bool RawNumber(const Ch* str, uint length, bool copy)
                                    { RAPIDJSON_SCHEMA_HANDLE_VALUE_(String, (CurrentContext(), str, length, copy), (str, length, copy)); }
    bool String(const Ch* str, uint length, bool copy)
                                    { RAPIDJSON_SCHEMA_HANDLE_VALUE_(String, (CurrentContext(), str, length, copy), (str, length, copy)); }
    ...
    bool Key(const Ch* str, uint len, bool copy) {
...
private:
    ...
    bool BeginValue() {
        ...
        if (schemaStack_.Empty())
            ...
            uint count = CurrentContext().patternPropertiesSchemaCount;
            ...
            if (count > 0) {
                ...
                uint& validatorCount = CurrentContext().patternPropertiesValidatorCount;
        ...
        if (!schemaStack_.Empty()) {
            ...
            if (hasher && context.valueUniqueness) {
                ...
                for (typename HashCodeArray::ConstValueIterator itr = a->Begin(); itr != a->End(); ++itr)
                    if (itr->GetUint64() == h) {
                        DuplicateItems(static_cast<uint>(itr - a->Begin()), a->Size());
    ...
    void AppendToken(const Ch* str, uint len) {
    ...
    void AddErrorInstanceLocation(ValueType& result, bool parent) {
        ...
        ValueType instanceRef(sb.GetString(), static_cast<uint>(sb.GetSize() / sizeof(Ch)),
                              GetStateAllocator());
    ...
    void AddErrorSchemaLocation(ValueType& result, PointerType schema = PointerType()) {
        GenericStringBuffer<EncodingType> sb;
        uint len = CurrentSchema().GetURI().GetStringLength();
        ...
        ValueType schemaRef(sb.GetString(), static_cast<uint>(sb.GetSize() / sizeof(Ch)),
            GetStateAllocator());
    ...
    void AddErrorArray(const ValidateErrorCode code,
        ISchemaValidator** subvalidators, uint count) {
        ValueType errors(kArrayType);
        for (uint i = 0; i < count; ++i)
