...
struct GenericStringRef {
     ...
     */
#endif
    GenericStringRef(const CharType* str, uint len)
    ...
    const uint length; //!< length of the string (excluding the trailing NULL terminator)

private:
    uint NotNullStrLen(const CharType* str) {
...
inline GenericStringRef<CharType> StringRef(const CharType* str, size_t length) {
    return GenericStringRef<CharType>(str, uint(length));
}
...
public:
    ...
    */
    ...
    GenericValue(const Ch* s, uint length) RAPIDJSON_NOEXCEPT : data_() { SetStringRaw(StringRef(s, length)); }

    //! Constructor for constant string (i.e. do not make a copy of string)
    explicit GenericValue(StringRefType s) RAPIDJSON_NOEXCEPT : data_() { SetStringRaw(s); }

    //! Constructor for copy-string (i.e. do make a copy of string)
    GenericValue(const Ch* s, uint length, Allocator& allocator) : data_() { SetStringRaw(StringRef(s, length), allocator); }
    ...
    */
    ...
    uint MemberCount() const { RAPIDJSON_ASSERT(IsObject()); return data_.o.size; }

    //! Get the capacity of object.
    uint MemberCapacity() const { RAPIDJSON_ASSERT(IsObject()); return data_.o.capacity; }
    ...
    */
    ...
    /*! \param newCapacity  The capacity that the object at least need to have.
        ...
        \note Linear time complexity.
    */
    GenericValue& MemberReserve(uint newCapacity, Allocator &allocator) {
    ...
    */
    ...
    uint Size() const { RAPIDJSON_ASSERT(IsArray()); return data_.a.size; }

    //! Get the capacity of array.
    uint Capacity() const { RAPIDJSON_ASSERT(IsArray()); return data_.a.capacity; }
    ...
    */
    ...
    /*! \pre IsArray() == true
        ...
        \see operator[](T*)
    */
    GenericValue& operator[](uint index) {
        RAPIDJSON_ASSERT(IsArray());
        RAPIDJSON_ASSERT(index < data_.a.size);
        return GetElementsPointer()[index];
    }
    const GenericValue& operator[](uint index) const { return const_cast<GenericValue&>(*this)[index]; }
    ...
    /*! \param newCapacity  The capacity that the array at least need to have.
        ...
        \note Linear time complexity.
    */
    GenericValue& Reserve(uint newCapacity, Allocator &allocator) {
        RAPIDJSON_ASSERT(IsArray());
    ...
    */
    ValueIterator Erase(ConstValueIterator first, ConstValueIterator last) {
        ...
        data_.a.size -= static_cast<uint>(last - first);
        return pos;
    }
    ...
    */
    uint GetStringLength() const { RAPIDJSON_ASSERT(IsString()); return DataStringLength(data_); }
    ...
    */
    GenericValue& SetString(const Ch* s, uint length) { return SetString(StringRef(s, length)); }
    ...
    */
    GenericValue& SetString(const Ch* s, uint length, Allocator& allocator) { return SetString(StringRef(s, length), allocator); }
...
private:
    ...
    static const uint kDefaultArrayCapacity = RAPIDJSON_VALUE_DEFAULT_ARRAY_CAPACITY;
    static const uint kDefaultObjectCapacity = RAPIDJSON_VALUE_DEFAULT_OBJECT_CAPACITY;

    struct Flag {
#if RAPIDJSON_48BITPOINTER_OPTIMIZATION
        char payload[sizeof(uint) * 2 + 6];     // 2 x SizeType + lower 48-bit pointer
    ...
    struct ShortString {
        ...
        inline static bool Usable(uint len) { return                       (MaxSize >= len); }
        inline void     SetLength(uint len) { str[LenPos] = static_cast<Ch>(MaxSize -  len); }
        inline uint GetLength() const       { return  static_cast<uint>(MaxSize -  str[LenPos]); }
    ...
    static RAPIDJSON_FORCEINLINE uint DataStringLength(const Data& data) {
    ...
    RAPIDJSON_FORCEINLINE Member* DoAllocMembers(uint capacity, Allocator& allocator) {
        return Malloc<Member>(allocator, capacity);
    }

    void DoReserveMembers(uint newCapacity, Allocator& allocator) {
        ObjectData& o = data_.o;
        if (newCapacity > o.capacity) {
...
#endif
        o.size -= static_cast<uint>(last - first);
        return pos;
    }
    ...
    void SetArrayRaw(GenericValue* values, uint count, Allocator& allocator) {
        data_.f.flags = kArrayFlag;
        if (count) {
    ...
    void SetObjectRaw(Member* members, uint count, Allocator& allocator) {
        data_.f.flags = kObjectFlag;
        if (count) {
    ...
    bool StringEqual(const GenericValue<Encoding, SourceAllocator>& rhs) const {
        ...
        const uint len1 = GetStringLength();
        const uint len2 = rhs.GetStringLength();
...
public:
    ...
    bool Key(const Ch* str, uint length, bool copy) { return String(str, length, copy); }
...
public:
    ...
    uint Size() const { return value_.Size(); }
    uint Capacity() const { return value_.Capacity(); }
    bool Empty() const { return value_.Empty(); }
    void Clear() const { value_.Clear(); }
    ValueType& operator[](uint index) const {  return value_[index]; }
    ValueIterator Begin() const { return value_.Begin(); }
    ValueIterator End() const { return value_.End(); }
    GenericArray Reserve(uint newCapacity, AllocatorType &allocator) const { value_.Reserve(newCapacity, allocator); return *this; }
...
public:
    ...
    operator ValueType&() const { return value_; }
    uint MemberCount() const { return value_.MemberCount(); }
    uint MemberCapacity() const { return value_.MemberCapacity(); }
    ...
    GenericObject MemberReserve(uint newCapacity, AllocatorType &allocator) const { value_.MemberReserve(newCapacity, allocator); return *this; }
