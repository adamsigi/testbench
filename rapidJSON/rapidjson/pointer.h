...
static const uint kPointerInvalidIndex = ~uint(0);  //!< Represents an invalid index in GenericPointer::Token
  ...
  //! A token is the basic units of internal representation.
    ...
    */
    struct Token {
        ...
        uint length;            //!< Length of the name.
        uint index;             //!< A valid array index, if it is not equal to kPointerInvalidIndex.
    };
    ...
    */
    ...
    /*!
        ...
        \return A new Pointer with appended token.
    */
    GenericPointer Append(const Ch* name, uint length, Allocator* allocator = 0) const {
    ...
    */
    ...
    /*!
        ...
        \return A new Pointer with appended token.
    */
    GenericPointer Append(uint index, Allocator* allocator = 0) const {
        char buffer[21];
        char* end = sizeof(uint) == 4 ? internal::u32toa(index, buffer) : internal::u64toa(index, buffer);
        uint length = static_cast<uint>(end - buffer);
        buffer[length] = '\0';
    ...
    */
    GenericPointer Append(const ValueType& token, Allocator* allocator = 0) const {
        if (token.IsString())
            ...
            RAPIDJSON_ASSERT(token.GetUint64() <= uint(~0));
            return Append(static_cast<uint>(token.GetUint64()), allocator);
        }
    }
    ...
    */
    ...
    void Parse(const Ch* source, size_t length) {
        ...
        while (i < length) {
            ...
            token->length = static_cast<uint>(name - token->name);
            if (token->length == 0)
                isNumber = false;
            *name++ = '\0'; // Null terminator
            ...
            if (isNumber) {
                for (size_t j = 0; j < token->length; j++) {
                    uint m = n * 10 + static_cast<uint>(token->name[j] - '0');
